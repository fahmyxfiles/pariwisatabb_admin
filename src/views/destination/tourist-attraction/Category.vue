<template>
  <div>
    ...
    <b-overlay :show="modalLoading" spinner-variant="primary" rounded="sm">
      <b-form>
        <b-form-group>
          <label for="touristAttractionName">Tourist Attraction Category Name :</label>
          <b-form-input
            id="touristAttractionName"
            v-model="params.name"
            type="text"
            placeholder="Tourist Attraction Category Name"
          />
        </b-form-group>
        <b-form-group>
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.loading = true
      this.$http
        .get('/tourist_attraction_category', { params: this.query })
        .then(res => {
    ...
    addModal() {
      this.initDefaultParams()
      this.modalTitle = 'Add Tourist Attraction Category'
    ...
    addData(params) {
      ...
      this.$http
        .post('/tourist_attraction_category', params)
        .then(res => {
    ...
    editModal(item) {
      this.initDefaultParams()
      this.modalTitle = `Edit Tourist Attraction Category : ${item.name}`
    ...
    editData(id, params) {
      ...
      this.modalLoading = true;
      this.$http
        .post(`/tourist_attraction_category/${id}`, params)
        .then(res => {
          this.$refs['modal-input'].hide()
          this.getData()
        })
    ...
    deleteData(item) {
      this.$swal({
        title: 'Are you sure?',
        text: `Tourist Attraction Category ${item.name} will be removed. All Tourist Attraction with this Category need to be updated with new Category.`,
        icon: 'warning',
        ...
        if (result.value) {
          this.$http
            .delete(`/tourist_attraction_category/${item.id}`)
            .then(res => {
              this.$swal({
                icon: 'success',
                title: 'Deleted!',
                text: 'Tourist Attraction Category has been deleted.',
                customClass: {
